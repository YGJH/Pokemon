...
# Full 5-stage pipeline: Rust → mine → shards → train(+baselines) → RL
...
| `deck_prior.py` | Torch-free opponent-deck determinization: `ArchetypePosterior` (Bayesian categorical over *every* archetype), the observation accumulator, and the caller-facing `OpponentDeckPredictor`. Used by the Kaggle bundle, `live_eval` and `ptcg_rl.train`; replaced the learned belief heads. |
...
- **Skill is a weight, not a filter, and only the *own*-deck archetype filters.** Two of the three corpus filters are gone. Measured on the 52.2k-episode corpus, the old top-K expert filter kept **8027 of 104418 (episode, player) pairs — 7.7%**, and the 𝒟_opp half of the deck filter dropped ~30% of what survived. `--k-experts` and `--g-min` still choose the experts `ptcg_mine` uses to *rank* 𝒟_self/𝒟_opp; they no longer decide which rows exist.
  ...
  **The 𝒟_opp half of the deck filter was the cheap one to give up, and the asymmetry is deliberate.** Nothing the model is trained on is defined in terms of 𝒟_opp — the labels are the expert's chosen option and the game outcome, both of which are exactly as valid against an off-list opponent. (When it was lifted, the one target that *was* defined that way, the belief head's `bel_arch`, already abstained via `ignore_index=-1`; the heads are gone now, so even that qualifier is retired.) Dropping the row instead trains nothing and removes the hard matchups a specialist most needs. `self_ids` has no such escape and is still enforced: a row whose *own* deck is not the one being trained is a different game. `build_shards(filter_opp=True)` reproduces the old corpus.

  One trap this hit, still live: **pass B filters independently of pass A**. It recomputes `opp_arch` and used to `continue` on `None`, which would silently reinstate the filter one pass after it was lifted, for exactly the decks that cluster to nothing. `meta.archetype_opp` keeps the opponent's real global cluster id — free to store, useful for slicing eval — and `-1` there means "matched no cluster at all", *not* "outside 𝒟_opp"; an off-list but clustered opponent keeps its id.
...
- **Archetype ids are append-only within a lineage, and only within one.** They are cluster indices assigned in the order the greedy pass opens clusters, which follows the global deck-frequency ordering — so an *unseeded* re-mine renumbers clusters whose membership never changed, and `--archetype-self N` silently becomes a different deck. `ptcg_mine.mine` therefore seeds `cluster_decks` from `<out-dir>/archetypes.json` **by default**: baseline clusters keep their id and their representative, unmatched decks open new clusters past the baseline maximum, and a baseline cluster with no members in the new corpus is retained at frequency 0 rather than freeing its id. `--rebaseline` is the explicit opt-out that renumbers and invalidates every trained checkpoint; `--baseline-archetypes PATH` seeds from elsewhere. `archetypes.json` records which generation it belongs to under `lineage` (`seeded`, `baseline_sha1`, `generation`).
  ...
  **`opp_ids` is append-only metadata now, and nothing is shaped by it.** It used to be the belief head's class order — the label was `{gid: i for i, gid in enumerate(opp_ids)}`, a *position* rather than an id, so a re-sorted list of unchanged length loaded without complaint and repointed every class. That whole failure mode left with the head, and with it `--allow-belief-widening`. `select_self_opp` still appends rather than re-ranks (`archetype._append_only`) and `n_opp` still reads as "how many of this corpus's top archetypes may be *added*", but a growing list no longer changes any checkpoint's shape: mining prints it as a note, and warm-starting across the boundary needs nothing special. The determinizer that replaced the head (`ptcg_il/deck_prior.py`) deliberately hypothesises over **every** archetype, not `opp_ids`, so it is not affected either.
...
- **Opponent-deck determinization is a Bayesian prior, not a learned head.** `ptcg_il/deck_prior.py` scores `P(deck_j | observed) ∝ P(deck_j) · Π_c P(c | deck_j)` over every archetype in `archetypes.json` and hands the search determinizer one 60-card template. No torch, no training, no shard labels. Two things carry it: the mined `frequency` prior is heavily skewed (top cluster = 37% of 153,702 player-slots), so with *zero* cards seen the argmax already shares **44.6/60** cards with the opponent's real deck against **19.4/60** for the mirror heuristic it replaces; and any card the opponent plays that archetype *j* does not run is decisive evidence against *j*, so the posterior collapses to **57.1/60 by four cards seen, 59.6/60 by twenty**. Counting is multiset-aware (a third copy of a 2-of is evidence), and `DEFAULT_EPSILON` keeps one mis-parsed observation from permanently zeroing the right hypothesis.

  **The hypothesis set is every archetype, deliberately not `opp_ids`.** The nine `opp_ids` the retired belief heads classified over cover only 68.7% of games by frequency, so a *perfect* classifier over them caps at ~52/60 — the ceiling was in the label space, not in the model. That is the core reason four supervised heads (`bel_arch`/`bel_deck`/`bel_hidden`/`bel_hand`), their `bel_*` shard labels, the `log_feat` game-log tensors, the `BeliefModule` encoder and the `--belief-*` / `--no-belief` / `--allow-belief-widening` flags are all gone. Live A/B on `checkpoints_a1_s2/ckpt-best.pt`, 200 games/opponent (`docs/superpowers/specs/2026-08-12-prior-determinizer-results.md`): `search_planner_prior` 1.4% vs `search_planner` (mirror) 0.9%, zero OOV, no extra illegal actions — directional, CIs overlap at that sample size, and it shipped on "better or equal on every measured dimension", not on significance.

  **Checkpoints trained with the heads still load.** `load_policy_state` drops `history_gru.*`, `belief.*` and `belief_heads.*` (`_RETIRED_PREFIXES`) instead of raising, so a `.pt` from before the removal evaluates fine — it just measures a trunk that was trained *with* the auxiliary loss and the log encoder. Anything else unexpected in the state dict still raises, because a half-loaded policy scores like a plausible model and is randomly initialised.
...
- **Both build-shards passes are pooled, and every serial remainder has been folded into one of them.** Pass A (the projection scan) has been parallel for a while; pass B (parse → featurize) was serial, and lifting the corpus filters made it the whole job. Measured on the 54,281-episode corpus at the time: pass A **1.2 min** (0.022 s/ep across 16 cores) against pass B **~85 min** on one core — **~98% of the wall clock**. Within pass B: orjson 57%, `featurize` 31%, belief labels 7%, file read 5% — measured while belief labels were still written, so that 7% is now gone and the rest redistributes. Note what moved: `featurize` used to be 0.7% of the job, because pass B re-parsed 14.5k episodes to *use* 774, so its cost was parsing episodes it discarded. Now pass B is path-filtered up front and nearly every episode is kept, so the array code that was free is a third of the dominant pass.
...
- **Every checkpoint records its own architecture.** `Policy.config` holds `D/heads/layers/ff/n_all_cards/feat_dims` (plus `attn_dropout`/`ffn_dropout` as provenance, deliberately *not* read back by `policy_from_config`), and `save_checkpoint` writes it. RL loads two policies (θ and the frozen π_IL) and must not infer shapes: build from the record, never from a guess. `D` fails loudly on mismatch, but the class of bug worth fearing is the quiet one — this is why `feat_dims` is pinned (a `featurizer.py` edit silently redefines every checkpoint and shard) and why `load_policy_state` raises on any unexpected key that is not a retired-module prefix.

- **The anti-deck-out guard lives at inference, in the mask** (`ptcg_il/deck_guard.py`, self-contained, vendored verbatim into `model/`).  Archetype 16 wins 63.3% of its corpus games yet ~10% of its losses are deck-outs.  Mechanism A hard-masks options whose resolution leaves the deck empty before the next-turn draw (`delta < 0 & deck + delta <= 0`, MAIN selects only), exempting last-prize KO attacks (`opt_scalar[7]`), with a never-empty floor so argmax never sees an all-dead mask.  Mechanism B re-ranks options within `margin` logits of the top by net deck delta, gated on `deck <= 10 & hand > 6`, and never overrides a KO attack on top.  The explicit burn table exists because `card_draw_counts` only parses "draw" phrasing: search cards (Poké Pad et al.) read 0, Judge is deliberately skipped, and Lillie's reads draw-6 while missing the shuffle-back — the wrong sign exactly in the big-hand states where B fires.  Scope matters the same way: a Pokémon's burn belongs to its *ability* (SKILL option), not to playing it — the episode replay caught PLAY Drakloak being charged −1, and the deck-0 over-masking that a missing `delta < 0` condition caused.  Thresholds are measured, not tuned: `deck_low=10` is p90 of per-turn net deck consumption over the full 93,609-game corpus (1.75M turn transitions; median 2, mean 4.0, p95 14); `margin=0.25` ≈ p25 of the masked top-2 logit-gap distribution across 2,226 gated arch-16 val decisions, where 13.3% of gated picks change and every inspected example moves a burn to a refill/neutral play.  Replay of 12 arch-16 deck-out losses: 11 are attrition (e.g. Comfey's Flower Shower −3 at deck 4, then the turn-start draw — correctly *not* masked, not being certain-death plays) and 1 is an immediately-fatal play (Lillie's at 6 prizes, draw 8), which the mask catches; the packaged bundle verified end-to-end on that exact decision.  `hand_gate=6` is the Lillie's reflow threshold per design.
