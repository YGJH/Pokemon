...
- **Archetype ids are not stable — never hardcode them.** They are cluster indices, so re-running mining reassigns them. The pipeline shipped `ARCHETYPES="0 2"` while this corpus's `self_ids` are `[0, 1, 11, 3, 4, 5]` — archetype 2 does not exist here. `ptcg_il/archetype_select.py` derives them from `archetypes.json` ∩ `meta.parquet`, ranked by training rows and filtered to those with enough held-out data to be model-selected. `run_pipeline.sh` calls it; so should you.
