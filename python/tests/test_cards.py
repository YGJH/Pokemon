...
def test_card_static_row_shape(engine, attacks_by_id):
    ...
    # 52 base + 29 ability kw + 2 counts + 2 card draw counts + 3 attacks × 46
    assert row.shape == (223,)
...
def test_build_static_tables_shapes_and_pad_row(engine):
    ...
    assert attack_table.shape[1] == 46
    assert attack_table.dtype == np.float32
    np.testing.assert_array_equal(attack_table[0], np.zeros(46, dtype=np.float32))
