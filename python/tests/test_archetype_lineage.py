...
# ── Widening the belief head across a seeded generation ─────────────────────
...
def test_widening_copies_the_old_slots_and_leaves_the_new_ones_untrained():
    import torch

    from ptcg_il.model.policy import load_policy_state, widen_belief_arch_head

    old, new = _policy(3), _policy(5)
    old_sd = old.state_dict()
    patched, w_old, w_new = widen_belief_arch_head(new, old_sd)
    assert (w_old, w_new) == (3, 5)

    key = "belief_heads.arch_head.2.weight"
    torch.testing.assert_close(patched[key][:3], old_sd[key])
    torch.testing.assert_close(patched[key][3:], new.state_dict()[key][3:])

    # And the load itself goes through.
    load_policy_state(new, old_sd, allow_belief_widening=True)
    torch.testing.assert_close(new.state_dict()[key][:3], old_sd[key])


def test_widening_is_off_by_default():
    """A shape mismatch is the only signal that 𝒟_opp moved; swallowing it by
    default would let a re-baselined corpus load a checkpoint whose slots mean
    different decks."""
    from ptcg_il.model.policy import load_policy_state

    old, new = _policy(3), _policy(5)
    with pytest.raises(RuntimeError):
        load_policy_state(new, old.state_dict())


def test_a_shrunken_head_raises_even_with_widening_allowed():
    """𝒟_opp got smaller, which append-only cannot produce — so the artifacts
    were re-clustered and the leading rows now describe different decks."""
    from ptcg_il.model.policy import load_policy_state

    wide, narrow = _policy(5), _policy(3)
    with pytest.raises(RuntimeError, match="shrank"):
        load_policy_state(narrow, wide.state_dict(), allow_belief_widening=True)


def test_widening_is_a_noop_at_equal_width():
    from ptcg_il.model.policy import widen_belief_arch_head

    a, b = _policy(4), _policy(4)
    sd = a.state_dict()
    patched, w_old, w_new = widen_belief_arch_head(b, sd)
    assert patched is sd and (w_old, w_new) == (0, 0)
